...
test("live market display prefers combined TheRundown plus PropLine rows", async () => {
  const todayJson = {
    date: "2026-04-28",
    generated_at: "2026-04-28T23:00:00Z",
    pitchers: [
    ...
    liveMarketDisplay: {
      ...
      rows: [
        ...
        },
        {
          ...
          book_rows: [
            { bookmaker_key: "fanduel", bookmaker_title: "FanDuel", line: 5.5, odds: -118 },
            { bookmaker_key: "betmgm", bookmaker_title: "BetMGM", line: 5.5, odds: -112 },
            { bookmaker_key: "betrivers", bookmaker_title: "BetRivers", line: 5.5, odds: 104 },
          ],
        },
      ],
    },
  ...
  assert.equal(row.best_book, "BetRivers");
  assert.equal(row.best_odds, 104);
  assert.equal(row.book_rows.length, 3);
});
