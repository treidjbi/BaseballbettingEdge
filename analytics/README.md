...
# Only picks since the last formula change
python analytics/performance.py --since 2026-04-08

# Only FIRE-tier EV (3%+)
python analytics/performance.py --min-ev 0.03
```
...
- **ev_vs_actual.png** — predicted EV bucket vs realized win rate. The single
  most important view: if the model's edge is real, higher EV buckets should
...
- **By EV bucket** — does higher predicted EV → higher realized ROI?
