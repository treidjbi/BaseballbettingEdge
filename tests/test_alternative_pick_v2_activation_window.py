...
def _artifact(*, game_time="2026-07-22T20:00:00+00:00", verdict="LEAN", date="2026-07-22", source="therundown", line_source_provider="therundown"):
    return {
        ...
        "pitchers": [{
            ...
            "market_source_mode": source, "line_source_provider": line_source_provider,
...
def test_activation_preflight_accepts_explicit_provider_but_rejects_missing_provider():
    explicit = preflight.evaluate_activation_window(
        payload=_artifact(line_source_provider="therundown"), raw_bytes=b"artifact", observed_at=NOW,
    )
    missing = preflight.evaluate_activation_window(
        payload=_artifact(line_source_provider=None), raw_bytes=b"artifact", observed_at=NOW,
    )

    assert explicit["status"] == "clean"
    assert missing == {**missing, "ok": False, "status": "invalid_artifact"}
