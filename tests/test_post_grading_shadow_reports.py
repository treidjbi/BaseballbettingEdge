...
def _stub_clv_runner_dependencies(tmp_path, monkeypatch):
    calls = []
    ...
    def fake_clv_main(argv):
        ...
        (output_path / "clv_process_target_validation.json").write_text(
            json.dumps(
                {
    ...
    return calls, output_dir, dataset_path, preclose_output


def test_runner_reports_missing_compact_clv_input_as_proxy_failed(tmp_path, monkeypatch, capsys):
    calls, output_dir, _, preclose_output = _stub_clv_runner_dependencies(tmp_path, monkeypatch)
    missing_market_input = tmp_path / "missing_market_pick_evidence.json"
    ...
    assert "CLV process target: coverage --; strong lift --; current-provider drift --; readiness proxy_failed." in output
    assert "keep_as_process_kpi" not in output


def test_runner_skip_flag_omits_clv_invocation_and_summary(tmp_path, monkeypatch, capsys):
    calls, output_dir, _, preclose_output = _stub_clv_runner_dependencies(tmp_path, monkeypatch)
