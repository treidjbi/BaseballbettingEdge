create table if not exists public.shadow_provider_movement_events (
  ...
  provider_latency_seconds integer,
  raw_payload jsonb not null default '{}'::jsonb,
  dedupe_key text not null unique,
  created_at timestamptz not null default now()
);

alter table public.shadow_provider_movement_events enable row level security;

create index if not exists shadow_provider_movement_events_source_observed_idx
  on public.shadow_provider_movement_events (source, observed_at desc);

create index if not exists shadow_provider_movement_events_match_idx
  on public.shadow_provider_movement_events (slate_date, market_key, normalized_player_name, side, observed_at desc);

create table if not exists public.shadow_movement_source_comparisons (
  id uuid primary key default gen_random_uuid(),
  slate_date date,
  market_key text,
  normalized_player_name text,
  ...
  primary_event_id uuid references public.shadow_provider_movement_events(id) on delete cascade,
  comparison_event_id uuid references public.shadow_provider_movement_events(id) on delete set null,
  primary_observed_at timestamptz not null,
  comparison_observed_at timestamptz,
  latency_delta_seconds integer,
  primary_current_line numeric,
  comparison_current_line numeric,
  same_line boolean,
  primary_current_odds integer,
  comparison_current_odds integer,
  odds_delta integer,
  primary_price_change_pct numeric,
  comparison_result text not null,
  would_reduce_spend_signal boolean not null default false,
...
alter table public.shadow_movement_source_comparisons enable row level security;

create index if not exists shadow_movement_source_comparisons_result_idx
  on public.shadow_movement_source_comparisons (comparison_result, primary_observed_at desc);

create index if not exists shadow_movement_source_comparisons_match_idx
  on public.shadow_movement_source_comparisons (slate_date, market_key, normalized_player_name, side);
...
begin
  ...
  with upserted as (
    ...
    where ms.provider in ('boltodds', 'propline')
      ...
      and ms.market_key = 'pitcher_strikeouts'
    on conflict (dedupe_key) do update set
