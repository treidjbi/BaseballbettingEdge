...
| --- | --- | --- |
| Pipeline / infrastructure | The production Alt recorder remains the isolated V1 sidecar at the end of the existing `bbe-live-layer` cycle; the code default is still `off`, and an unset/blank bundle-version key still selects V1. Dependency-aware V2 core is locally verified at `6955a1b5`, but it is not merged or deployed. The 2026-07-23 live isolation read found migration `20260722230000_alternative_pick_v2_evaluation_proof.sql` absent, the `evaluation_proof` column absent, zero V2 rows, `50` V1 rows, and zero alternative notification events. Protected V1, workflow, provider, model, artifact, lock, and history path checks exited zero. | Finish the independent whole-branch review, then push only the two held branches without merging. Keep the prospective fixture, migration, Render merge/deploy, V2 activation, and Netlify merge/deploy gates closed. Immediate sidecar stop remains `ALTERNATIVE_PICK_SELECTION_MODE=off` plus a `bbe-live-layer` redeploy; do not change cadence or provider posture. |
| Model | V1 remains the only production comparison selector, with frozen fingerprint `f8f7ccf652b8eda4860d07798bc4673920b0b9d727552bc7e2e6547d478b4579`. The dependency-aware V2 implementation is locally implemented and integration-verified with frozen fingerprint `23bacff0fa923685ae52c5a9cfbadfb9f5902fb64d91759cfe9b4b1169a221c4`; core verification passed `375` focused and `1,816` full Python tests plus the existing V1 endpoint/full Node suites. The exploratory official-close `148-84`, `+38.585u`, `+16.63%` on `232` remains hindsight-capable research only. V2 has zero prospective production rows, and the missing July 22 exact artifact was not reconstructed. | Capture and independently review the next clean prospective exact fixture before Production Gate A. Do not infer readiness from local tests or historical PnL. Official pick selection, model math, staking, thresholds, notifications, locks, providers, shared analytics, retention, and source truth remain closed. |
| UI | Live Netlify still serves the V1 endpoint/browser contract. The backward-compatible V1/V2 endpoint and selected-with-pending browser work are independently Task-7-clean on held branch `codex/alt-picks-dependency-aware-v2-web` at `ba6b11c1`; held verification passed `52` focused and `126` full Node tests, `1,816` full Python tests, syntax checks, and desktop/mobile visual QA. That branch is unmerged and undeployed, so Picks, Results, accepted bets, and the production Alt tab remain on their existing behavior. | Keep the web branch held until a complete prospective V2 candidate cycle passes the later runtime gate. Retain deploy `6a602de17d040836c1641df0` as the immediate endpoint rollback target, `6a60e7c06cebf50c3948c690` as the reviewed V1 Alt UI target, and `6a5e5bbef33863243b980d83` as the pre-Alt UI target. |
