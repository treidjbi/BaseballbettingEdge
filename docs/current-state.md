...
- As of 2026-07-07, the mainline best-price notification policy is implemented
  in code and verified locally, but it remains default-`off` and not promoted
  to `shadow` or `send`. Local verification on the
  `codex/mainline-best-price-notifications` worktree passed:
  ...
  `20` passing tests, and `git diff --check` returned clean. The requested
  `npx supabase db push --linked --dry-run` did not execute because this
  worktree is not linked to a Supabase project and the CLI stopped at
  `Cannot find project ref. Have you run supabase link?`. No migration apply,
  Render env change, deploy, push, or shadow/send rollout was executed. Keep
  `LIVE_MAINLINE_PRICE_NOTIFICATION_MODE=off` until Tyler separately approves a
  linked dry run, shadow env flip, and Render verification.
