...
| Tracking / data collection / history | `docs/research/market-tracker-map.md`, `docs/research/pitcher-k-outcome-dataset.md`, `2026-06-07-market-agreement-tracker.md`, `2026-08-18-season-retention-foundation.md`, `2026-08-18-bounded-retention-audit-design.md`, `2026-08-18-bounded-retention-audit.md`, compact outcome outputs, live-market audits | Canonical research and market/live/provider trackers remain active. Season-retention Phase 1 is complete and merged on local `main` at `c74385a7`; the exact reporter fails closed on contradictory coverage, incomplete provider/runtime equality, stale Phoenix-day evidence, missing decision linkage/pins, and incomplete retired-BoltOdds preservation. The former monolithic live reads remain rejected after PostgreSQL `53100` and `57014`. Static/local bounded-audit tooling is independently reviewed and merged locally on `main` at `5b0cf17cec6d9c1950dcebad045221e49e95e782`: fixture-only 1/3/7-day checkpoints, runtime boundary, local assembly, and both closed reporters. The final whole-branch review fix blocks every BoltOdds readiness partition with `post_suspension_runtime_evidence` when any correctly flagged current run, snapshot, heartbeat, or message maximum is after suspension, while closure still reports `operational_exception`; pre-suspension BoltOdds, all non-Bolt providers, v1 behavior, evidence, and pins remain unchanged. A 2026-08-19 linked sizing read reports `4,914,924,691` database bytes (about `4.58 GiB`, `57.2%` of 8 GiB) and `3,449,225,216` bytes / approximately `3.93M` estimated rows in `market_snapshots`. Gate 1 proved the one-date query/checkpoint path. Gate 2's approved `boltodds` / `2026-06-11` stress query found `7,139` raw snapshots / `5,414,880` logical bytes across `340` raw and compact groups, only `336` exact; `4` groups disagree on last-seen/last-odds/movement/snapshot counts and `157` observed-window rows link to a different provider-run slate date. Gate 3's separately approved runtime-boundary query completed in `16.03` seconds and passed integrity, freshness, ordering, and redaction validation. BoltOdds current and candidate maxima are identical and pre-suspension: latest run/heartbeat/message at `2026-06-17T17:20:59Z`, latest snapshot `2026-06-16T13:37:44Z`, and `post_boltodds_suspension=false`; there is no database evidence of accidental reactivation. The approved Gate 2 diagnosis found a real late-write preservation gap. A separately approved BoltOdds-only backfill then repaired exactly `233` compact groups across May 30, June 10, and June 11. Post-write proof reconciles all `33,588` raw snapshots and `1,743` compact groups exactly, with zero missing, unexpected, mismatched, or non-compactable rows; all `157` cross-date snapshots now have compact group, source-ID, and time-bound coverage. The version 2 preserved-lineage contract is merged on local `main` at `7507cebd`: it reports total, preserved, and unpreserved cross-date counts; only unpreserved rows block, and the exact sum equation fails closed. Fresh new-hash checkpoints now pass: April 28 is exact zero-row evidence; June 11 reconciles `7,139` rows and `340/340` groups with `157` preserved / zero unpreserved cross-date rows; the current runtime boundary remains clean with no post-suspension BoltOdds activity. Gate 4 invocation 1 added exact zero-row checkpoints for April 29–May 1 and May 2–4. Gate 4 invocation 2 reconciled May 5–10 and proved `4,800` cross-date rows preserved with zero unpreserved. Gate 4 invocation 3 used its full five-query cap across May 11–25: `210,501` raw snapshots and all `11,121` raw groups have exact compact matches, while `20,176` cross-date rows are preserved and zero are unpreserved. May 17–18 has `855` unexpected compact groups (`220` plus `635`). The first three diagnoses proved their source/run lineage, the exact May 17 `pitcher_strikeouts` versus `Strikeouts` alias, and the bounded May 18 heartbeat/carryover cohorts. The final aggregate equivalence gate did not pass for the extras themselves: `117` May 17 groups differ only on movement count and `35` May 18 groups contain duplicate listed source IDs. Preservation nevertheless passes on the true dates: all `855/855` underlying actual-date groups, containing `25,220` raw snapshots, have exact compact matches, and all `21,761/21,761` source IDs listed by the extras map to those canonical groups. The extras are redundant noncanonical historical artifacts, not season truth. Local code review identifies unstable timestamp-only ordering/offset pagination and absent duplicate handling as the high-confidence code-path cause; no further database diagnosis is needed. Eleven historical checkpoints plus runtime validate structurally; exact coverage under the current generic contract and the full matrix remain incomplete. Nothing was deleted, reclaimed, pushed, deployed, or changed in production behavior. | Gates 1-3, the compact repair, preserved-lineage integration, Gate 4 invocations 1-3, unexpected-compact diagnoses 1-3, and the final equivalence read are complete. Before BoltOdds May 26–28, separately design and test stable `(observed_at, id)` compaction ordering, duplicate handling, and a date/provider/alias-bounded historical-extra rule that requires exact actual-date preservation. Retention activation, deletion, repair/normalization/reclassification, audit weakening, vacuum, storage reclamation, provider/model/notification/lock/UI changes, push, and deployment remain closed. |

### August 20 retention-compaction handoff board overlay

| Lane | Current stage | Next decision / blocker |
| --- | --- | --- |
| Tracking / data collection / history | The deterministic compaction and bounded historical-extra hardening is implemented locally on `codex/retention-compaction-historical-extras` through reviewed commits `525bea29`, `598f5369`, `ebe21b20`, `19cd05fc`, and `1607befe`. It uses deterministic `(observed_at, id)` source order, duplicate-ID fail-closed handling, strict `coverage_exact`, and separately recomputed `retention_preservation_complete`; the exact runner-v3 query-contract hash is `a222b8608ea39ffb6e00d7254361b8ce8672c5f10a6f55e7342be92e6c82fa0a`. Task 3's temporary reporter failure was a sequencing seam resolved by Task 4. No live query, mutation, retention execution, deletion, repair, backfill, deployment, push, or merge occurred. | Local merge review first. Only after a separately approved merge may a fresh bounded audit be considered; strict coverage and all preservation gates must be re-evaluated from fresh evidence. Retention activation, deletion, vacuum, storage reclamation, provider/model/notification/lock/UI changes, push, and deployment remain closed. |
