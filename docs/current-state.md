...
the Supabase rollup provider constraints were widened for `therundown`. This
is not an official-provider env flip: latest 2026-06-19 parity is safer but
still not ready, with line conflicts excluded (`0.0%`) and provider coverage
down to `22/29` (`75.9%`). Keep `OFFICIAL_MARKET_SOURCE=therundown` and
continue observing until the coverage and ready-row gates pass on clean slices.
