...
| --- | --- | --- | --- |
| Pipeline / infrastructure | `2026-05-19-supabase-operational-foundation.md`, `2026-05-22-github-artifact-exit.md`, `2026-05-13-boltodds-propline-official-provider-cutover.md`, `2026-05-20-live-notification-coordinator.md`, `2026-06-04-live-notification-digest-coordinator.md`, `docs/operational-risk-register.md` | Render + Supabase artifact API are the primary scheduler/artifact path as of 2026-05-30. GitHub scheduled triggers are disabled, but manual `workflow_dispatch` remains the rollback path. As of 2026-06-02, Render preview/full/refresh run a non-strict BoltOdds + PropLine provider-source canary through `official_market_lines`; TheRundown remains fallback/rollback and strict mode remains off. Render lock and grading commands stay unchanged. Render preview, pipeline/full/refresh, and lock modes publish only the artifact keys each mode can actually regenerate; Render grading/full/refresh/lock hydrate from Netlify `get-artifact` before running so stale checkout copies of history, params, performance, preview lines, or the optional FanGraphs last-good cache are not republished. `bbe-pipeline-lock` runs every 10 minutes offset behind the live-layer lock ledger and can replay already-consumed Supabase lock rows if a prior stale artifact publish left the dashboard behind. Dashboard artifact adapters default to Netlify `get-artifact` with static fallback. As of 2026-06-04, Render `bbe-live-layer` is deployed on the notification digest coordinator commit and running `LIVE_NOTIFICATION_COORDINATOR_MODE=shadow` with start-window and pick-change grouping flags enabled; Supabase now allows digest event types, but user-facing sends remain individual. Line/price movement stays individual but now gets shadow provider-strength labels. FanGraphs SwStr and batter aggregate K% fetches now write/read `data/fangraphs_cache.json`; preview/full/refresh publish it as optional `fangraphs_cache` artifact so future 403s can reuse fresh last-good stats before falling fully neutral. As of 2026-06-05, hosted Supabase also allows `fangraphs_cache` in `published_pipeline_artifacts`, after a schema allow-list mismatch caused the 2026-06-05 00:37Z and 01:07Z Render refresh/final publish failures. Render pipeline cron services keep autoDeploy off; after an approved main push, use `python scripts/deploy_render_pipeline_crons.py --execute` to redeploy preview, grading, full, refresh, and lock as one validated group. | Observe the next scheduled preview/full/refresh after the Supabase artifact allow-list repair: no 400s on artifact publication, `fangraphs_cache` stays 200 when fresh, and `today`/dated/steam/picks_history stay fresh. Also keep watching provider attribution in `today.json`/dated archive/steam, no lock regressions, no duplicate/wrong-date publishes, and TheRundown fallback/manual GitHub rollback still available. For notifications, review at least one real shadow digest opportunity before enabling `LIVE_NOTIFICATION_COORDINATOR_MODE=grouped`. Keep model, staking, thresholds, retention deletion, default dashboard UI, and strict provider mode separate. |
| Model | `2026-05-12-pitcher-k-outcome-research-dataset.md`, `2026-05-29-gate-ef-under-fire-conversion-shadow-plan.md`, `2026-06-03-market-favorite-confidence-referee-shadow-plan.md`, `2026-06-03-gate-f-projection-challenger-shadow-plan.md`, `2026-06-05-market-favorite-confidence-referee-production-canary.md` | Gate C durable artifact is now the preferred full-corpus research input. The refreshed market-favorite confidence-referee lab has enough evidence for Tyler to approve a canary plan despite being `16` validation tracked rows short of the old threshold: validation rows are `234 < 250`, while `market_favorite_referee_candidate` is `114` rows, `72-42`, `+11.80`, `+10.4%`; `model_fades_market_favorite` is negative at `113` rows, `51-62`, `-7.07`, `-6.3%`. As of 2026-06-05, phase-one confidence-referee code is deployed to Render pipeline crons with `MARKET_FAVORITE_REFEREE_MODE=shadow`. The first post-shadow refresh published 18 metadata rows, 0 applied caps, and 0 live verdict changes; because the first lock had already happened, 2026-06-05 is only a partial same-day smoke. The Gate F projection package still blocks all lambda challengers: shrink variants are directionally useful but below MAE-lift threshold, and handedness remains hindsight-only. Gate E/F under-skepticism remains promising but not promotion-ready because the current selected candidate retained zero FIRE 2u wins. | Observe the next full slate with `MARKET_FAVORITE_REFEREE_MODE=shadow`, then review the canary audit before any `enforce` discussion. This remains verdict-conversion work only. Do not change lambda, global thresholds, staking, provider order, notifications, locks, retention, dashboard source-of-truth, or Path B handedness from these reports. |
