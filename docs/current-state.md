...
| Model | Versioned disjoint Consensus Core and Re-entry Expansion selectors are live for prospective comparison with frozen fingerprint `f8f7ccf652b8eda4860d07798bc4673920b0b9d727552bc7e2e6547d478b4579`. The exploratory official-close `148-84`, `+38.585u`, `+16.63%` on `232` remains research only, can include post-T30 information, and is not a prospective sample or promotion signal. Twelve current provisional rows now have five to 45 exact bound observations, but selected rows remain zero: strict preclose/provider completeness and same-line consistency checks still fail closed on missing or conflicting provider evidence. Three exact production checkpoints are frozen and honestly pending. | Continue prospective comparison without loosening provider consistency. Keep official model, staking, thresholds, notifications, locks, providers, and source truth closed; use the five-slate and 25/75 frozen-pick review gates before any separate proposal. |
| UI | Netlify production deploy `6a60e7c06cebf50c3948c690` serves the isolated same-day Alt Picks tab and sanitized endpoint. After the repaired cycle, the endpoint returned `ready` with 15 current provisional rows, three frozen rows, 12 mature evidence rows, zero selected, and 18 pending. In-app-browser QA showed the intended read-only pending state and confirmed Picks, Results, and accepted-bet behavior remain isolated. No Netlify redeploy was needed. | Keep observing selected-card evidence when a lane actually qualifies. Treat any proposal to relax provider completeness or same-line consistency as a separate model/evidence-governance review; retain deploy `6a602de17d040836c1641df0` as the immediate endpoint rollback target and `6a5e5bbef33863243b980d83` as the pre-Alt UI rollback target. |
