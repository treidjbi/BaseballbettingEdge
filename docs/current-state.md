...
through `f39f438b`. The dependency-aware V2 runtime binding repair is merged at
`014a7338`, and the exact ten-path V2 endpoint/UI release is merged at
`280d5d8b`. Production recording and display are open only for isolated
prospective comparison state; every official model, verdict, staking,
notification, lock, provider, accepted-bet, artifact, history, and
source-of-truth promotion gate remains closed.

| Lane | Current stage | Next decision |
| --- | --- | --- |
| Pipeline / infrastructure | V2 is active only in the existing post-lock `bbe-live-layer` sidecar. Commit `014a7338` reconstructs `slate_date` in memory from the already date-scoped parent provider run, aligns the effective official quote, and preserves exact pending dependency reasons; it adds no schema, backfill, worker, cadence, or provider change. Safe-code deploy `dep-d9h6cijeo5us73bcqnl0` and one-key activation deploy `dep-d9h6d33eo5us73bcrfpg` are live. The complete Render environment changed from `19` to `20` keys with only `ALTERNATIVE_PICK_SELECTION_BUNDLE_VERSION=v2`. Normal `19:10Z` and `19:20Z` cycles succeeded. | Observe the first real V2 frozen-pregame checkpoints and bounded row growth. Never reconstruct the already-missed Shane Bieber checkpoint. Immediate sidecar stop remains `ALTERNATIVE_PICK_SELECTION_MODE=off` plus a `bbe-live-layer` redeploy; do not change cadence or provider posture. |
| Model | The dependency-aware V2 comparison selector is live with frozen fingerprint `23bacff0fa923685ae52c5a9cfbadfb9f5902fb64d91759cfe9b4b1169a221c4`. At `19:20Z`, three prospective rows had fresh exact TheRundown evidence: Randy Dobnak UNDER 3.5 was selected in `consensus_core` with three agreeing families and four observations; Brandon Pfaadt OVER 3.5 and Michael McGreevy OVER 3.5 were not selected with two agreeing families and six observations each. The exploratory official-close `148-84`, `+38.585u`, `+16.63%` on `232` remains hindsight-capable research only. | Soak prospective V2 selections through T-30 freezes and graded outcomes. This is comparison evidence, not approval for official pick selection, model math, thresholds, staking, providers, notifications, locks, retention, or source truth. |
| UI | Netlify production deploy `6a6269fe216598bb9cbe7394` serves the versioned V2 endpoint and Alt tab from web commit `280d5d8b`. Unversioned and explicit V1 requests still serve the exact V1 bundle; explicit V2 returned three rows and one selected card; invalid versions fail closed; raw proofs and observation identifiers stay private. Desktop and `390x844` checks showed the Dobnak Consensus Core card, two collapsed supporting rows, accurate evidence chips/detail, no wager controls, and no horizontal overflow. Picks still showed eight official props and Results still showed `1,648` graded picks. | Keep the Alt page comparison-only and observe the provisional-to-frozen label transition. Retain `6a60e7c06cebf50c3948c690` as the reviewed V1 Alt UI rollback target and `6a5e5bbef33863243b980d83` as the pre-Alt UI target. |
| Tracking / data collection / history | The additive V2 proof schema is active. Current V2 state is exactly three provisional rows, zero duplicate groups, zero proof mismatches, and a maximum proof size of `4,622` bytes. Alternative notification rows stayed `0`; operational locks stayed `3/3` unique and consumed; accepted bets stayed at `2`; all three V1 frozen rows remain. The official artifact stayed byte-identical at SHA-256 `058379766c921b7d2183c66bfeb3b99aa992e985071df61036a1073f4e2365ad`, generated `19:08:04Z`, with TheRundown as every row's line source. | Continue bounded prospective collection. Do not backfill missed checkpoints, delete/compact Alt evidence, or let Alt rows enter official history/analytics without a separate approval. Version rollback must restore the exact original bundle-key state and prove a clean V1 cycle before restoring a V1 UI. |
