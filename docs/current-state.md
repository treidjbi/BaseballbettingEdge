...
| Tracking / data collection / history | `docs/research/market-tracker-map.md`, `2026-08-18-bounded-retention-audit.md`, `docs/research/2026-08-21-boltodds-retention-closure-preflight.md`, `2026-08-20-retention-compaction-and-historical-extras-design.md`, `2026-08-20-retention-compaction-and-historical-extras.md`, compact outcome outputs, live-market audits | Canonical research and market/live/provider trackers remain active. The documented `21` continuous BoltOdds checkpoints cover April 28-July 9, the repaired June 16 checkpoint is exact at `107/107`, and fresh July 10-16 plus July 17-22 checkpoints prove all `13` later partitions are exact zero rows/runtime/anomalies. The retired-provider candidate contains about `440 MiB` of raw logical payload backed by about `27.3 MiB` of compact evidence; the database is about `4.70 GiB`, or `58.75%` of the included 8 GiB. Deletion readiness remains blocked: the committed Gate C manifest omits decision-linked June 3; `652/844` tracked rows lack explicit provider metadata; normalized season-evidence/pin manifests are absent; and the latest completed physical backup predates the repair. June 3 evidence is not lost: Supabase history preserves the Gasser result, but unfiltered Netlify `picks_history` now exceeds the response ceiling. A tested local-only fix adds inclusive history date windows and routes Gate C through them. | Separately review, merge, push, and deploy the bounded history transport, then run a production-shaped Gate C rebuild and verify June 3/full reconciliation. Next resolve provider attribution or explicit unknown pins, generate aggregate-only evidence/pin manifests, assemble the canonical checkpoint envelope, and verify a completed post-repair backup. Retention execution, deletion, vacuum, reclamation, and production promotion remain closed; no provider/model/notification/lock/UI/source-of-truth behavior changed. |
