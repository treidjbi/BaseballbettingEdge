...
| Tracking / data collection / history | `docs/research/market-tracker-map.md`, `2026-08-18-bounded-retention-audit.md`, `docs/research/2026-08-21-boltodds-retention-closure-preflight.md`, `2026-08-20-retention-compaction-and-historical-extras-design.md`, `2026-08-20-retention-compaction-and-historical-extras.md`, compact outcome outputs, live-market audits | Canonical research and market/live/provider trackers remain active. The documented `21` continuous BoltOdds checkpoints cover April 28-July 9, the repaired June 16 checkpoint is exact at `107/107`, and fresh July 10-16 plus July 17-22 checkpoints prove all `13` later partitions are exact zero rows/runtime/anomalies. The full retired-provider candidate scope contains `611,972` raw rows / `461,536,160` logical bytes (about `440 MiB`) backed by `26,726` compact rows / `28,600,016` logical bytes, with zero raw rows after the suspension boundary. Current database size is about `4.70 GiB`, or `58.75%` of the included 8 GiB. Deletion readiness remains blocked: Gate C covers only `40/41` BoltOdds-era dates and omits decision-linked June 3; `652/844` tracked rows lack explicit provider metadata; no normalized season-evidence/pin manifest is present; and the latest completed physical backup predates the repair. | Repair the June 3 season record and provider attribution, generate aggregate-only evidence/pin manifests, assemble the canonical checkpoint envelope, and verify a completed post-repair backup. Only then draft a separate exact bounded-deletion proposal showing expected reusable—not guaranteed immediately returned—space. Retention execution, deletion, vacuum, reclamation, push, merge, and deployment remain separate closed gates; no provider/model/notification/lock/UI/source-of-truth behavior changed. |
