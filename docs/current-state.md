...
| Tracking / data collection / history | `docs/research/market-tracker-map.md`, `2026-08-18-bounded-retention-audit.md`, `2026-08-20-retention-compaction-and-historical-extras-design.md`, `2026-08-20-retention-compaction-and-historical-extras.md`, compact outcome outputs, live-market audits | Canonical research and market/live/provider trackers remain active. Retention compaction hardening and the May 18 carryover correction are merged and pushed on `main` at `2e5299aa`. Fresh read-only runner-v3 evidence covers nine validated BoltOdds checkpoints through May 19 plus the retired-runtime boundary. The May 17-19 checkpoint reproduced `855` known historical extras and isolated the corrected false negative affecting `610/635` May 18 carryover groups. One separately approved post-fix read under query hash `c95139e3` reached the unchanged 120-second ceiling, produced no checkpoint, and was not retried. The reviewed explicit one-chunk selector and exact set-based source-membership optimization are now merged into local `main` at `ebef1f39`; focused verification passed `431` tests and the complete merged-tree suite passed `2,448`. Local `main` has not been pushed, and no post-optimization linked read, database mutation, deletion, vacuum, or reclamation has occurred. | Decide separately whether to push local `main`. After publication, any one-chunk linked read under query hash `e3091876` requires another separate approval and a fresh explicit output directory. Complete the full provider/date matrix, recoverability proof, and exact recovery estimate before any separately approved deletion or vacuum. Provider/model/notification/lock/UI behavior remains unchanged. |
