...
| Tracking / data collection / history | `docs/research/market-tracker-map.md`, `2026-08-18-bounded-retention-audit.md`, `2026-08-20-retention-compaction-and-historical-extras-design.md`, `2026-08-20-retention-compaction-and-historical-extras.md`, compact outcome outputs, live-market audits | Canonical research and market/live/provider trackers remain active. Retention compaction hardening is merged and pushed on `main` at `f6550337`. Fresh read-only runner-v3 evidence now covers nine validated BoltOdds checkpoints through May 19 plus the retired-runtime boundary. The May 17-19 checkpoint reproduced `855` known historical extras: all `220` May 17 extras passed preservation, while `610/635` May 18 extras were falsely rejected because the historical class required a May 17 observation date even when preserved sources were observed May 18 against the pinned May 17 run. One aggregate-only diagnostic isolated that predicate with zero unresolved sources or missing runs. A test-first local correction on `codex/retention-may18-carryover-fix` allows only May 17-18 observations for this exact carryover class and changes the query hash to `c95139e3`; no post-fix live read, database mutation, deletion, or reclamation has occurred. | Review and commit the isolated predicate fix, then request separate merge approval. After merge, request a separate fresh linked audit under the new hash; every old-hash checkpoint remains invalid. Complete the full provider/date matrix, recoverability proof, and exact recovery estimate before any separately approved deletion or vacuum. Provider/model/notification/lock/UI behavior remains unchanged. |
