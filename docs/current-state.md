...
| UI | `2026-05-20-live-market-decision-ui.md`, `2026-05-20-live-notification-coordinator.md`, `2026-06-04-live-notification-digest-coordinator.md` | Phase 2 hidden market-decision MVP exists behind `?marketSheet=1`: `dashboard/v2-data.js` still accepts sanitized `window.V2_LIVE_MARKET_DISPLAY` fixture rows, and now fetches `/.netlify/functions/live-market-display` only when the hidden flag is present. The Netlify function reads `live_market_display_state` with server-side Supabase credentials and returns only app-safe allow-listed rows plus sanitized `book_rows` / `movement_events`. `dashboard/v2-app.jsx` shows the actionable-card market strip and detail-sheet market panel. This does not make the dashboard default-on and does not change provider/source-of-truth, model, threshold, staking, lock, retention, or notification behavior. | Review the hidden live market sheet with real same-day rows, then decide whether it needs display polish, accepted-bet prefill, or a later default-on review. Keep default-on UI, provider promotion, and betting-rule changes separate. |
