...
| Tracking / data collection / history | `docs/research/market-tracker-map.md`, `2026-08-18-bounded-retention-audit.md`, `2026-08-20-retention-compaction-and-historical-extras-design.md`, `2026-08-20-retention-compaction-and-historical-extras.md`, compact outcome outputs, live-market audits | Canonical research and market/live/provider trackers remain active. Retention compaction hardening, the May 18 carryover correction, and the explicit one-chunk/set-membership optimization are published on GitHub `main`; the current query-contract SHA is `e3091876`. Fresh read-only evidence covers `21` continuous BoltOdds checkpoints from April 28 through July 9 and confirms no retired-runtime rows after June 16. Tyler approved the exact June 16 repair: one bounded upsert corrected the `17/107` stale/truncated compact groups under preview fingerprint `320c254d`, with no delete or other table mutation. A separate fresh checkpoint now proves all `107/107` groups exact, complete preservation, zero missing/mismatched/unexpected/duplicate groups, and zero source anomalies; the raw footprint remains only `131,160` logical bytes. The post-repair July 10-16 linked checkpoint failed closed once as `subprocess_failed`; it wrote no checkpoint and was not retried. No space was reclaimed, and no provider/model/notification/lock/UI/source-of-truth behavior changed. | The June 16 compact-integrity blocker is closed. Resume the remaining read-only BoltOdds/provider matrix only after a clean linked-read window, beginning with July 10-16; then estimate recoverable space and verify season-evaluation pins before proposing any deletion rule. Retention execution, deletion, vacuum, reclamation, push, merge, and deployment remain separate closed gates. |
