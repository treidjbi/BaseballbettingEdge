...
| Tracking / data collection / history | `docs/research/market-tracker-map.md`, `2026-08-18-bounded-retention-audit.md`, `2026-08-20-retention-compaction-and-historical-extras-design.md`, `2026-08-20-retention-compaction-and-historical-extras.md`, compact outcome outputs, live-market audits | Canonical research and market/live/provider trackers remain active. Retention compaction hardening, the May 18 carryover correction, and the explicit one-chunk/set-membership optimization are published on GitHub `main`; the current query-contract SHA is `e3091876`. Fresh read-only evidence covers `21` continuous BoltOdds checkpoints from April 28 through July 9 and confirms no retired-runtime rows after June 16. June 16 remains the first blocker: `17/107` compact groups are stale/truncated, omitting `28` earlier snapshots while retaining exact last observations; its raw footprint is only about `0.125` MiB. The first approved repair-preview read reached the linked database but failed closed before producing a report because same-provider/same-slate heartbeat evidence crossed the Phoenix-day boundary. Local commit `e6416fc1` now quarantines and counts those out-of-window heartbeats without expanding repair lineage, binds the counts into preview fingerprint version `2`, and uses a source-state digest to invalidate post-write proof if inputs change. Verification is `40` focused repair tests, `204` combined repair/retention tests, and `2,476` full-suite tests, with independent rereview clean. Two SELECT-only wrapper attempts under `e943b90d`, one simplified preview attempt under `062aafdc`, and one metadata-only hosted-schema attempt under `639f6cfb` exited once with code `1`; none was retried or changed the database. A separately approved linked `select now()` then exited successfully, proving connectivity; only its temporary local decoder rejected the successful CLI envelope before retaining the timestamp. The schema wrapper now accepts the observed envelope families and persists a redacted error excerpt on failure. Deletion, vacuum, reclamation, push, deployment, and production behavior remain unchanged. | Do not query application tables again yet. Request separate approval for one replacement metadata-only hosted-schema read under unchanged hash `639f6cfb`, then use its retained schema/error evidence before drafting another preview. Repair/upsert, recoverability proof, recovery estimate, retention execution, deletion, and vacuum remain closed. Provider/model/notification/lock/UI behavior is unchanged. |
