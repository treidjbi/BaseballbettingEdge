...
| Tracking / data collection / history | `docs/research/market-tracker-map.md`, `2026-08-18-bounded-retention-audit.md`, `2026-08-20-retention-compaction-and-historical-extras-design.md`, `2026-08-20-retention-compaction-and-historical-extras.md`, compact outcome outputs, live-market audits | Canonical research and market/live/provider trackers remain active. Retention compaction hardening, the May 18 carryover correction, and the explicit one-chunk/set-membership optimization are published on GitHub `main`; the current query-contract SHA is `e3091876`. Fresh read-only evidence now covers `21` continuous BoltOdds checkpoints from April 28 through July 9 and confirms no retired-runtime rows after June 16. June 16 is the first blocker: `17/107` compact groups are stale/truncated, omitting `28` earlier snapshots while retaining exact last observations; its raw footprint is only about `0.125` MiB. One approved aggregate-only diagnosis isolated that shape without a write. A separately reviewed code-only repair-preview command is local on `codex/retention-june16-repair-preview` at `8e619bb1`; `37` focused and `2,473` full-suite tests pass. It is hard-limited to the exact BoltOdds June 16 partition, preview-fingerprint gated, upsert-only, and deletion-incapable. No live repair preview, database mutation, deletion, vacuum, reclamation, push, deployment, or production behavior change has occurred. | Next, request separate approval for one read-only June 16 repair preview in a fresh output directory. Review its aggregate counts and exact fingerprint before any separately approved upsert. Then require an exact June 16 retention checkpoint before resuming the remaining BoltOdds/provider matrix. Recoverability proof, recovery estimate, retention execution, deletion, and vacuum all remain closed. Provider/model/notification/lock/UI behavior is unchanged. |
