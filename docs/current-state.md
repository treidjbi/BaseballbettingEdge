...
| UI | `2026-05-20-live-market-decision-ui.md`, `2026-05-20-live-notification-coordinator.md`, `2026-06-04-live-notification-digest-coordinator.md` | Phase 1 hidden market-decision MVP exists behind `?marketSheet=1`: `dashboard/v2-data.js` accepts sanitized `window.V2_LIVE_MARKET_DISPLAY` rows, attaches them to matching pitcher/side rows, and `dashboard/v2-app.jsx` shows an actionable-card market strip plus detail-sheet market panel. This is fixture/preloaded-data only and does not read live Supabase from the browser or make the dashboard default-on. | Review the hidden fixture-backed UI, then decide whether to build the Phase 2 sanitized Netlify live-market endpoint. Keep display work separated from provider promotion and betting-rule changes. |
