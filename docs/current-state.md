...
July 22 first-slate repairs are merged through `be462774`. The
...
| --- | --- | --- |
| Pipeline / infrastructure | The post-T30 sidecar is live at the end of the existing live-layer cycle, after notifications, live state, operational locks, timing, and shadow-market work. The code default remains `off`; Tyler-approved production `record` mode is isolated to `bbe-live-layer`. The July 22 adjacent-date TheRundown overlap was fixed by deduplicating raw provider event IDs before both event and snapshot normalization. Live-layer deploy `dep-d9ge756rnols73ch9c60` runs commit `256aaa22`; scheduled 15:40Z, 15:50Z, and 16:00Z captures completed with 29 unique events from 30 raw rows, no further Postgres `21000`, the unchanged two-request cadence, and 323-329 datapoints per cycle. The 17:10Z cycle completed with fresh remote artifact state, zero notification events, and one due lock inserted. | Keep observing normal scheduled cycles and bounded row volume. Roll back only this sidecar with `ALTERNATIVE_PICK_SELECTION_MODE=off` plus a live-layer redeploy if any boundary fails; do not change cadence or provider posture from this repair. |
| Model | Versioned disjoint Consensus Core and Re-entry Expansion selectors are live for prospective comparison with frozen fingerprint `f8f7ccf652b8eda4860d07798bc4673920b0b9d727552bc7e2e6547d478b4579`. The exploratory official-close `148-84`, `+38.585u`, `+16.63%` on `232` remains research only, can include post-T30 information, and is not a prospective sample or promotion signal. Two exact production checkpoints are now frozen, both honestly `pending` with no lane selected; this proves collection integrity but is not a selected-pick performance sample. | Continue the prospective comparison. Keep official model, staking, thresholds, notifications, locks, providers, and source truth closed; use the five-slate and 25/75 frozen-pick review gates before any separate proposal. |
| UI | Netlify production deploy `6a60e7c06cebf50c3948c690` serves the isolated same-day Alt Picks tab and sanitized endpoint. The endpoint repair maps an explicit allow-list of canonical MLB full names to safe display codes without changing database identities. Production first returned all 17 provisional rows; after artifact advancement it returned both valid frozen rows with `artifact_advanced_after_freeze=true`. Pending/provisional/frozen payload QA passed; no selected row existed to exercise selected-card copy. Picks, Results, and accepted-bet behavior remain unchanged. | Keep observing normal selected-card evidence when a lane actually qualifies; retain deploy `6a602de17d040836c1641df0` as the immediate endpoint rollback target and `6a5e5bbef33863243b980d83` as the pre-Alt UI rollback target. |
| Tracking / data collection / history | Migration `20260721222627_alternative_pick_selection_state.sql` is applied with RLS/browser denial and frozen-row validation/immutability triggers. The first normal slate produced 20 bounded provisional rows and two immutable `frozen_pregame` rows. Both frozen rows exactly matched their `operational_pick_locks` across candidate identity, line, price, book, game time, timing, artifact SHA/path, and team metadata; both ledger rows were consumed, duplicate alternative keys were zero, and alternative notification events were zero. No retention behavior changed. | Continue bounded collection without backfill or reconstruction. Do not delete or compact this evidence, and do not treat pending frozen checkpoints as selected-pick performance, without separate approval. |
