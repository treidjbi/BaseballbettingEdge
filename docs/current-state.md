...
| --- | --- | --- |
| Pipeline / infrastructure | The Martin Perez / Martín Pérez duplicate was traced to exact Unicode display-name uniqueness in history hydration and seeding. The scoped repair now deduplicates by normalized `(date, pitcher, side)`, preserves graded or locked evidence, and retains the current display spelling. New regression tests plus the broader results/pipeline suite pass. Render CLI authentication was refreshed and the intended Tyler workspace was verified without changing service settings. | Merge and deploy the repair to the seven Render pipeline cron services, then require a fresh scheduled artifact to show one normalized Pérez identity with source/model/lock behavior unchanged. |
