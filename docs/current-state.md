...
| Tracking / data collection / history | `docs/research/market-tracker-map.md`, `2026-08-18-bounded-retention-audit.md`, `2026-08-20-retention-compaction-and-historical-extras-design.md`, `2026-08-20-retention-compaction-and-historical-extras.md`, compact outcome outputs, live-market audits | Canonical research and market/live/provider trackers remain active. Retention compaction hardening, the May 18 carryover correction, and the explicit one-chunk/set-membership optimization are published on GitHub `main`; the current query-contract SHA is `e3091876`. Fresh read-only evidence covers `21` continuous BoltOdds checkpoints from April 28 through July 9 and confirms no retired-runtime rows after June 16. June 16 remains the first blocker: `17/107` compact groups are stale/truncated, omitting `28` earlier snapshots while retaining exact last observations; its raw footprint is only about `0.125` MiB. Local commit `e6416fc1` quarantines cross-day heartbeats without expanding lineage and binds source-state changes into preview/post-write proof. After isolating Windows inline-SQL transport loss, the linked metadata read succeeded through CLI `--file` and confirmed all `63` expected hosted columns/types. The exact June 16 read-only preview then succeeded at query hash `2f88ae78`: `285` provider runs, `791` heartbeats (`580` in-window, `211` quarantined), `172` raw snapshots, and `107` rebuilt/existing compact groups. Exactly `17` groups are mismatched, with `0` missing and `0` unexpected; the blocker-free preview would upsert `17` rows under fingerprint `320c254d`. No database write, deletion, vacuum, reclamation, push, deployment, or production behavior change occurred. | Stop before mutation. The next materially different gate is a bounded `17`-row upsert using exact preview fingerprint `320c254d`, followed by fresh source-state and exactness verification. The upsert itself would reclaim no space. Repair/upsert, recoverability proof, recovery estimate, retention execution, deletion, and vacuum remain closed until separately authorized. Provider/model/notification/lock/UI behavior is unchanged. |
