...
- Also on 2026-06-10, `PROFIT_RESCUE_REFEREE_MODE=off|shadow|enforce` was
  implemented as a downgrade-only FIRE exposure canary but not promoted.
  Default remains `off`; `shadow` adds metadata only; `enforce` can only lower
  remaining FIRE exposure after quality and confidence-referee caps. The first
  profit-rescue audit showed strong downside-control evidence, but enforce mode
  is still a separate Tyler approval decision.
