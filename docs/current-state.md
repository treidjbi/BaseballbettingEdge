...
4. Read the newest active dated plans that match the task:
   - `docs/superpowers/plans/2026-09-04-bounded-forward-capture-proposal.md`
     for the completed prototype and passive receipt review: end-of-cycle hooks
     cannot resolve by-lock acquisition; instrumentation deferred, hosted capture closed
   - `docs/superpowers/plans/2026-09-04-signed-score-proof-repair-review.md`
     for the approved paired Python/JavaScript repair at `b08284fd` and its
     implementation acceptance; feature branch only, not deployed
   - `docs/superpowers/plans/2026-09-04-forward-evidence-validator.md`
     for completed offline acceptance; no capture, runner integration or formal credit
   ...
   - `docs/superpowers/plans/2026-07-29-no-drag-and-strict-runtime-prospective-review.md`
     ...
     prospective boundary, and the completed independent decision
   - `docs/research/2026-08-10-no-drag-strict-runtime-decision.md` for the
     August 19 evidence decision to retire no-drag v1 from the active promotion
     path and keep strict runtime as a narrow OVER/minus specialist review only
...
Use [the concise companion brief](operating-brief.md) for the current decision
summary and [the detailed assessment](research/2026-09-04-operating-and-research-assessment.md)
for frozen candidate contracts, costs, historical/prospective distinctions,
and the retained primary evidence. This overlay updates next decisions only;
the detailed plans, historical board and prior overlays remain preserved.
Recommendations to pause or retire recurring attention do not change running
jobs, data collection, production behavior, or any promotion gate.
...
| Tracking / data collection / history | The approved [offline decision-time adapter](research/2026-09-04-decision-time-adapter-review.md) is implemented on a feature branch: 352 bounded identities, 309 linked records, 23 frozen selective-LEAN rule matches, 19 linked matches and zero formal credit. All 327 captured Preclose proofs remain pending; archive/frozen matches overlap 21/23. Historical Gate C hashes are explained by CRLF, and original files remain preserved. Cleanup remains 82/82 complete; last storage read ~71.52%. Kumar's lock remains unconsumed and excluded. | The [offline forward-evidence validator](research/2026-09-04-forward-evidence-validator-review.md) is complete: 76 new tests, 357 combined tests, 20 synthetic scenarios, zero formal credit. The approved [paired signed-score repair](research/2026-09-04-signed-score-implementation.md) is implemented at `b08284fd`: 539 Python and 124 Node passes; 27 PostgreSQL tests skipped for missing binaries. New inactive manifest: 20 synthetic cases, nine input candidates, zero credit; old packets preserved and all 327 old proofs stay pending. The [offline capture prototype](research/2026-09-04-forward-capture-feasibility.md) is complete: 35 focused / 207 combined tests, seven of 16 synthetic internal passes, and a 12-opportunity September 3 historical run with zero complete inputs. Its 12 recorded consumed locks lack the required pregame receipts; all Preclose proofs remain pending. Stop before hosted capture; pause repeated historical runs. The [completed passive receipt review](research/2026-09-04-passive-receipt-review.md) confirms that end-of-cycle hooks cannot resolve by-lock eligibility: all five market receipt families must be acquired by the earlier lock clock. Eight synthetic boundary cases pass their expected outcomes; no new credit. Defer instrumentation and hosted work. Revisit only on materially corrected complete by-lock sources with original-seed continuity, or a separately commissioned runtime/evidence-version redesign; no automatic implementation follows. The [capture plan](superpowers/plans/2026-09-04-bounded-forward-capture-proposal.md) retains limits and rollout gates. Feature branch only; no deployment or back-credit. Capture inactive; activation unset. Preserve book conflicts, baselines and review gates; Kumar remains excluded. No production, history, retention or scheduler change. |
...
| Tracking / data collection / history | `docs/research/market-tracker-map.md`, `2026-08-18-bounded-retention-audit.md`, `2026-08-25-active-provider-compaction-finalizer.md`, `2026-08-26-daily-active-provider-compaction-finalizer.md`, `2026-08-27-manual-active-provider-compaction-intervals.md`, `docs/research/2026-08-21-boltodds-retention-closure-preflight.md`, `2026-08-20-retention-compaction-and-historical-extras-design.md`, `2026-08-20-retention-compaction-and-historical-extras.md`, compact outcome outputs, live-market audits | Canonical research and market/live/provider trackers remain active, and BoltOdds stays retired. The bounded Netlify history path is live. August 20 remains repaired at `11`, `7-4`, `+2.317154u`. The August 19 canonical slate is `18`, `8-10`, `-3.752883u`; all nine recovered lock-backed rows remain `data_complete=0`, and bounded Supabase/Netlify reads agree. The frozen Alt V2 record from the first healthy July 24 slate is `38-46`, `-16.751729u` on `84` graded rows, plus one void and zero missing. The August 25 SELECT-only retention audit completed all `360/360` April 28-July 26 provider/date partitions. Retired BoltOdds is exact, but stale 20,000-row compaction prefixes left `105` active-provider partitions requiring `34,861` aggregate upserts: PropLine `63/15,387` and TheRundown `42/19,474`. Main commit `2278c249` is live on `bbe-live-layer` as deploy `dep-da6vieap6svc73b5nvv0`. The first post-deploy compaction-due run finished successfully at `20:31:33Z`; it kept the explicit fail-closed compaction warning while producing `21` provisional Alt V2 rows instead of `prerequisite_failed`. A linked SELECT confirmed those `21` rows at `20:30:51Z`, while compact state remained unchanged at `876` rows / `20,000` represented snapshots with latest update `18:41:16Z`, proving no partial publication. Top-level/current/official/lock/timing/ready-to-bet failures remain blocking. The frequent live compactor ceiling remains unchanged. A narrow linked proof under query contract `611dd4eb...` classifies all `24` PropLine May 17 extras as exact May 16 carryover with zero unpreserved groups. The raw PropLine webhook inbox contains `440,717` rows/about `435.6 MiB`; it has no FK/view/trigger dependency, but active writes and `10` recent unprocessed rows were present. A completed physical backup at `2026-08-25T05:39:27.316Z` is newer than the repairs; PITR remains disabled. The exact active-provider finalizer is implemented, preview-first, and compact-only when separately gated. Tyler declined another paid Render cron on 2026-08-27 in favor of operator-run intervals. The manual extension accepts one validated clean-regime historical slate date, keeps the provider pair fixed, rejects ranges, and uses a write gate separate from the D-1 gate. The first operator-run tranche completed for `2026-06-12`: two stable previews identified `577` repairs, the separately approved execution wrote `208` PropLine and `369` TheRundown compact rows, and a fresh preview proved both providers exact with zero missing, mismatched, unexpected, or remaining upsert rows. The `2026-05-14` preview correctly failed closed because TheRundown source evidence was absent. The separately approved `2026-06-13` execution wrote `185` PropLine plus `591` TheRundown compact rows (`776` total), and the fresh post-write preview proved both providers exact with zero missing, mismatched, unexpected, or remaining upsert rows. The separately approved `2026-06-14` execution wrote `172` PropLine plus `266` TheRundown compact rows (`438` total), and a fresh post-write preview proved both providers exact with zero missing, mismatched, unexpected, or remaining upsert rows. The separately approved `2026-06-15` execution wrote `112` PropLine plus `314` TheRundown compact rows (`426` total), and a fresh post-write preview proved both providers exact with zero missing, mismatched, unexpected, or remaining upsert rows. The separately approved `2026-06-16` execution wrote `192` PropLine plus `500` TheRundown compact rows (`692` total), and a fresh post-write preview proved both providers exact with zero missing, mismatched, unexpected, or remaining upsert rows. The separately approved `2026-06-17` execution wrote `142` PropLine plus `322` TheRundown compact rows (`464` total), and a fresh post-write preview proved both providers exact with zero missing, mismatched, unexpected, or remaining upsert rows. The separately approved `2026-06-18` execution wrote `44` PropLine plus `224` TheRundown compact rows (`268` total), and a fresh post-write preview proved both providers exact with zero missing, mismatched, unexpected, or remaining upsert rows. The separately approved `2026-06-19` execution wrote `180` PropLine plus `450` TheRundown compact rows (`630` total), and a fresh post-write preview proved both providers exact with zero missing, mismatched, unexpected, or remaining upsert rows. The separately approved `2026-06-20` execution wrote `188` PropLine plus `534` TheRundown compact rows (`722` total), and a fresh post-write preview proved both providers exact with zero missing, mismatched, unexpected, or remaining upsert rows. The separately approved `2026-06-21` execution wrote `159` PropLine plus `397` TheRundown compact rows (`556` total), and a fresh post-write preview proved both providers exact with zero missing, mismatched, unexpected, or remaining upsert rows. The separately approved `2026-06-22` execution wrote `163` PropLine plus `447` TheRundown compact rows (`610` total), and a fresh post-write preview proved PropLine exact at `178` rows and TheRundown exact at `463` rows with zero missing, mismatched, unexpected, or remaining upsert rows. The next read-only `2026-06-23` preview passed with no evidence blockers and identified `198` PropLine plus `516` TheRundown compact repairs (`714` total); no write was attempted for that date. No deletion occurred; the remaining historical backlog and all raw-retention execution remain closed. | The next eligible tranche is the exact `2026-06-23` compact-only execution, pending separate Tyler approval; after execution require a zero-upsert post-write preview. Continue one historical date at a time. Recheck storage approximately every seven days, at 70 percent utilization, and at season end. Webhook deletion remains `NO-GO` until its separate gate passes. Raw retention deletion, vacuum/reclamation, receiver changes, and notification changes remain separate closed gates. |

### August 27 manual active-provider compaction completion overlay

This overlay supersedes the Four-Lane board's stale `2026-06-23` next action.
It changes only compact historical evidence; no raw row, webhook row, provider
usage, runtime behavior, or source-of-truth rule changed.
...
manual rollback after artifact exit. It also records the completed hydrated
Render repair and the first live storage review at the documented 70 percent
trigger. No provider, model, threshold, staking, notification, lock, dashboard,
retention, or source-of-truth behavior changed.

| Lane | Confirmed evidence | Next decision / blocker |
| --- | --- | --- |
| Pipeline / infrastructure | The September 2 scheduled Render grading run failed and published nothing. GitHub repair run `33650131043` then proved unsafe because it read stale repository history, selected May 30, and failed before push or publication. The authenticated hydrated Render service `bbe-pipeline-grading` was therefore triggered once and finished successfully at `2026-09-02T16:24:13Z`. Supabase publication run `manual-render-grading-2026-09-01-20260902T162358Z` completed with five artifacts: September 1 dated slate, index, params, performance, and picks history. Netlify `get-artifact` returned the same fresh hashes and `2026-09-02T16:23:58.854002Z` publication time. Tyler separately approved `bbe-gate-c-post-grading-review`. Its first manual run failed after the bounded evidence export when the 20-second public `index` read timed out; a direct follow-up returned HTTP 200 in `0.568s`. One bounded retry then completed successfully at `2026-09-02T17:09:52Z` in `5m55s`. Preview, full, lock, live-layer, and current-slate artifacts remained healthy. | Do not retry GitHub grading or patch only its staging list. A usable GitHub grading fallback needs a separate artifact-hydration design and proof. Return the research cron to ordinary observation; if remote artifact timeouts recur, consider a separate test-first bounded retry/backoff change rather than repeated operator runs. |
| Model | September 1 canonical locked outcomes are repaired: all `14/14` locked rows graded `6-8` for `-3.665684u`; the other nine history rows were unlocked and correctly remain outside performance scoring. Performance and params were regenerated at `2026-09-02T16:23:57Z` with `2,352` closed picks and a `1,887`-row complete-data calibration sample. The successful hosted research retry now covers `2026-04-28` through `2026-09-01` with `4,638` source rows and `2,436` tracked graded rows. No-drag remains fail-closed on baseline drift, while selective LEAN and strict runtime remain blocked on input/diversity gaps; strict runtime stays `20/50`, `15-5`, `+5.864u`, with zero UNDER or plus-price rows. | Keep every model/change gate closed. Treat the repaired official performance/history and refreshed research corpus as current evidence only; do not infer a Gate C, Alt V2, CLV, market-anchor, bet-selection, threshold, or staking promotion from it. |
...
`722` are parseable mainline movements, `12` are unsupported alt ladders,
...
Tyler approved the bounded repair of six missing August 20 history rows and the
oversized research-history loader. No provider order, live model, threshold,
staking, notification, lock, UI, retention, or source-of-truth rule changed.

| Lane | Confirmed evidence | Next decision |
| --- | --- | --- |
| Pipeline / infrastructure | Repair job `job-da4b6qk9v7es739ji1qg` published the grading scope with source `manual_backfill`. Research deploy `dep-da4bju0jo6nc739jdapg` is live on exact commit `1ff69ed5`; verification job `job-da4bkaqjnfac73cnm7gg` succeeded after one transient compact-table retry. The unchanged research command now stages the canonical history once instead of requesting the oversized full payload repeatedly. The bounded public August 20 history read returns `11` rows. | Observe the next natural grading and research runs. Escalate only on history divergence, another oversized full-history dependency, or repeated compact-table failures. |
| Tracking / data collection / history | The repaired August 20 official slate is `11`, `7-4`, `+2.317154u`; the six recovered rows are `3-3`, `-0.452083u`. All six persist with `data_complete=0`, and the next canonical read shows `3,247` total history rows. Performance and the dated archive include the recovered results; calibration stayed at `1,713` eligible rows and `lambda_bias=0.012`. | Keep the rows visible for performance but quarantined from calibration. Do not infer missing provenance fields after normal history normalization; the repair run and lock/proof audit remain the recovery provenance. |
| Model | The hosted rebuild completed with `4,204` source rows, `2,204` tracked rows, zero duplicates, and `2,135/2,135` reconciliation. Market-anchor strict displayed FIRE is the strongest watch signal at `47`, `29-18`, `+2.92u/+6.2%`; all strict tracked rows are only `+0.70u/+0.3%`, the side and critical-attribution gates are closed, and the decision remains `keep_shadow`. Selective LEAN has `0/75` eligible prospective rows; strict runtime remains `20/50`; market-shrink is negative and unapplied; no-drag is retired and now fail-closes on baseline drift with `48` recovered exclusions. | No promotion plan. Continue soak and repair only the research input/provenance gaps under their controlling plans. Gate C/D/E/F/12E and all behavior-changing gates remain closed. |
| UI | Unchanged. | Continue the separate Alt V2/default-on live-market UI soak. |

### August 19 no-drag and strict-runtime decision overlay

The delayed August 10 decision packet was issued on August 19 using the
canonical hosted research run and evidence through the fully graded August 18
slate. This closes only the two candidate decisions; it changes no runtime,
environment, model, provider, notification, lock, artifact, UI, history,
retention, or source-of-truth behavior.

| Lane | Confirmed evidence | Next decision |
| --- | --- | --- |
| Model | No-drag v1 is `retire` from the active promotion path after its complete 75-row prospective window went `41-34`, `-4.883u/-6.5%`; the latest 14 selected slate dates were `18-20`, `-7.361u`, and every leave-one-slate-out result remained negative. Keep the frozen selector/audit only as comparison/control. Strict runtime is `narrow_specialist_review`: current-provider history is `20/50`, `15-5`, `+5.864u`, but all rows are OVER/minus/FIRE 1u/pre-30, no rows have earned post-July-29 prospective credit, and provider/agreement attribution remains absent. | No no-drag or strict-runtime promotion plan. Do not create a replacement branch from the same hypothesis or weaken diversity/attribution gates. Gate C/D/E/F/12E and all live behavior gates remain closed; other candidates continue only under their own plans. |
| Pipeline / infrastructure | Existing post-grading audits remain read-only. This packet requires no deploy, service, schedule, command, environment, artifact, database, or provider change. | Ordinary research-run observation only. No operational action follows from the documentation decision. |
| UI | Unchanged. | Continue the independent Alt V2/default-on live-market UI soak. |
| Tracking / data collection / history | Preserve both frozen fingerprints, locked baselines, 25 history-recovered exclusions, and the dated decision packet. Missing provider/agreement attribution remains visible rather than inferred. | Retain the audits as low-cost controls; add no table, backfill, or retention exception for these candidates. |
