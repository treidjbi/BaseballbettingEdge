...
| Tracking / data collection / history | `docs/research/market-tracker-map.md`, `2026-08-18-bounded-retention-audit.md`, `2026-08-20-retention-compaction-and-historical-extras-design.md`, `2026-08-20-retention-compaction-and-historical-extras.md`, compact outcome outputs, live-market audits | Canonical research and market/live/provider trackers remain active. Retention compaction hardening is merged and pushed on `main` at `f6550337`. Fresh read-only runner-v3 evidence covers nine validated BoltOdds checkpoints through May 19 plus the retired-runtime boundary. The May 17-19 checkpoint reproduced `855` known historical extras and isolated a false negative affecting `610/635` May 18 carryover groups: their sources were observed May 18 while correctly linked to the pinned May 17 provider run. The bounded test-first correction allows only May 17-18 source observations for that exact class, keeps the May 17 run pin and all preservation proofs, and changes the query hash to `c95139e3`. Tyler approved option 1 and the reviewed fix was fast-forwarded into local `main` at `0bbc5957`; merged-tree verification passed `2,441` tests. Local `main` has not been pushed, and no post-fix live read, database mutation, deletion, vacuum, or reclamation has occurred. | Decide separately whether to push local `main`. A fresh linked audit is another separate approval and must run under query hash `c95139e3`; every old-hash checkpoint remains invalid. Complete the full provider/date matrix, recoverability proof, and exact recovery estimate before any separately approved deletion or vacuum. Provider/model/notification/lock/UI behavior remains unchanged. |
