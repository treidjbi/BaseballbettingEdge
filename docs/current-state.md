...
| --- | --- | --- |
| Pipeline / infrastructure | The production Alt recorder remains the isolated V1 sidecar at the end of the existing `bbe-live-layer` cycle; the code default is still `off`, and an unset/blank bundle-version key still selects V1. Dependency-aware V2 core is locally held at `ad6a0596`: `2ceaaa35` fails closed on malformed proof scalars, `e5c39523` requires chronological Preclose checkpoints, and `ad6a0596` isolates V2 startup imports. It is not merged or deployed. The 2026-07-23 live isolation read found migration `20260722230000_alternative_pick_v2_evaluation_proof.sql` absent, the `evaluation_proof` column absent, zero V2 rows, `50` V1 rows, and zero alternative notification events. Protected V1, workflow, provider, model, artifact, lock, and history path checks exited zero. | Final Sol Ultra whole-branch review is pending; only then may Task 9 push the two held branches without merging. Keep the prospective fixture, migration, Render merge/deploy, V2 activation, and Netlify merge/deploy gates closed. Immediate sidecar stop remains `ALTERNATIVE_PICK_SELECTION_MODE=off` plus a `bbe-live-layer` redeploy; do not change cadence or provider posture. |
| Model | V1 remains the only production comparison selector, with frozen fingerprint `f8f7ccf652b8eda4860d07798bc4673920b0b9d727552bc7e2e6547d478b4579`. The dependency-aware V2 implementation is locally held with frozen fingerprint `23bacff0fa923685ae52c5a9cfbadfb9f5902fb64d91759cfe9b4b1169a221c4`; current core full-Python verification passed `1,850` tests, but final Sol Ultra whole-branch review is pending. The exploratory official-close `148-84`, `+38.585u`, `+16.63%` on `232` remains hindsight-capable research only. V2 has zero prospective production rows, and the missing July 22 exact artifact was not reconstructed. | Capture and independently review the next clean prospective exact fixture before Production Gate A. Do not infer readiness from local tests or historical PnL. Official pick selection, model math, staking, thresholds, notifications, locks, providers, shared analytics, retention, and source truth remain closed. |
| UI | Live Netlify still serves the V1 endpoint/browser contract. The backward-compatible V1/V2 endpoint and selected-with-pending browser work are held separately on `codex/alt-picks-dependency-aware-v2-web` at `5ca07dc5`, including endpoint proof-scalar validation alignment. Held verification passed `60` focused and `134` full Node tests; final Sol Ultra whole-branch review remains pending. That branch is unmerged and undeployed, so Picks, Results, accepted bets, and the production Alt tab remain on their existing behavior. | Keep the web branch held until final review and a complete prospective V2 candidate cycle pass the later runtime gate. Retain deploy `6a602de17d040836c1641df0` as the immediate endpoint rollback target, `6a60e7c06cebf50c3948c690` as the reviewed V1 Alt UI target, and `6a5e5bbef33863243b980d83` as the pre-Alt UI target. |
