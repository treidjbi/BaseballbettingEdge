...
| Model | `2026-05-12-pitcher-k-outcome-research-dataset.md`, `2026-07-29-no-drag-and-strict-runtime-prospective-review.md`, `2026-07-29-market-anchor-downside-review.md`, `2026-07-29-strong-base-fire-policy-shadow-matrix.md`, `2026-07-29-market-shrink-retirement-decision.md`, `2026-07-29-clv-process-target-validation.md` | Gate C remains the canonical research input; active canaries remain bounded and do not approve live model, threshold, staking, provider, notification, lock, retention, or dashboard-source changes. As of the July 30 natural research run, no-drag is `79/75` and `ready_for_review`, but its diversity/slice gates remain open. Strict-runtime current-provider history is `20/50`, `15-5`, `+5.864u`, with zero UNDER and plus-price rows; prospective review starts July 30. Market-anchor is `keep_shadow` after its `48`-row paired downside cohort, Strong Base policies `cap_high_raw_edge` and `strict_runtime_core_flat` are frozen at `0/75`, and market-shrink remains `retain_diagnostic_shadow` with its betting-promotion path closed. Final CLV remains `keep_as_process_kpi`, but the runner is `proxy_failed` by default: no approved official-close packet producer exists, and refreshed `market_pick_evidence` movement exports are insufficient and cannot enter the target validator. | Keep all live gates closed. An explicit valid close-provenance packet is required before this diagnostic can produce eligible targets; then wait for 100 fully attributed current-provider CLV targets plus two positive complete 14-slate strong-proxy-lift windows before considering a separate proxy-design plan. Do not treat volume alone as promotion evidence. |
