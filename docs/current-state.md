...
| --- | --- | --- |
| Pipeline / infrastructure | The research-only post-grading runner now has an explicit `--refresh-market-agreement-inputs` mode. It reads bounded compact Supabase evidence, prebuilds agreement, builds Gate C once, then renders final agreement before downstream audits. Existing flag-off behavior is unchanged. Integrated verification on current `main` history returned `1,867` Python and `99` Node tests passing. Tyler explicitly approved the isolated research-cron credential/configuration deployment on `2026-07-23`; no other Render or production configuration is in scope. | Deploy and verify only `bbe-gate-c-post-grading-review`. Preserve the named Robert Gasser `2026-06-03` fail-closed exception and verify that no live pipeline, provider, notification, lock, UI, artifact, or retention behavior changes. |
...
| --- | --- | --- |
| Pipeline / infrastructure | The production Alt recorder remains the isolated V1 sidecar at the end of the existing `bbe-live-layer` cycle; the code default is still `off`, and an unset/blank bundle-version key still selects V1. Final reviewed core reference `8c29bc66` includes `2ceaaa35` fail-closed proof-scalar handling, `e5c39523` chronological Preclose checkpoints, and `ad6a0596` isolated V2 startup imports. It is not merged or deployed. The 2026-07-23 live isolation read found migration `20260722230000_alternative_pick_v2_evaluation_proof.sql` absent, the `evaluation_proof` column absent, zero V2 rows, `50` V1 rows, and zero alternative notification events. Protected V1 and official workflow/provider/model/artifact/lock/history paths are clean. | Final Sol Ultra whole-branch review is CLEAN with no Critical, Important, or Minor findings, so Task 9 may push the two held branches without merging. Keep the prospective fixture, migration, Render merge/deploy, V2 activation, and Netlify merge/deploy gates closed. Immediate sidecar stop remains `ALTERNATIVE_PICK_SELECTION_MODE=off` plus a `bbe-live-layer` redeploy; do not change cadence or provider posture. |
| Model | V1 remains the only production comparison selector, with frozen fingerprint `f8f7ccf652b8eda4860d07798bc4673920b0b9d727552bc7e2e6547d478b4579`. The dependency-aware V2 implementation is locally held with frozen fingerprint `23bacff0fa923685ae52c5a9cfbadfb9f5902fb64d91759cfe9b4b1169a221c4`; final core verification passed `1,850` Python and `99` Node tests, and all prior review findings are closed. The exploratory official-close `148-84`, `+38.585u`, `+16.63%` on `232` remains hindsight-capable research only. V2 has zero prospective production rows, and the missing July 22 exact artifact was not reconstructed. | Capture and independently review the next clean prospective exact fixture before Production Gate A. Do not infer readiness from local tests or historical PnL. Official pick selection, model math, staking, thresholds, notifications, locks, providers, shared analytics, retention, and source truth remain closed. |
| UI | Live Netlify still serves the V1 endpoint/browser contract. The backward-compatible V1/V2 endpoint and selected-with-pending browser work are held separately on `codex/alt-picks-dependency-aware-v2-web` at `5ca07dc5`, including endpoint proof-scalar validation alignment. Held verification passed `60` focused and `134` full Node tests plus `24` UI-isolation tests; final Sol Ultra review is CLEAN. That branch is unmerged and undeployed, so Picks, Results, accepted bets, and the production Alt tab remain on their existing behavior. | Task 9 may push the held web branch only; keep it unmerged until a complete prospective V2 candidate cycle passes the later runtime gate. Retain deploy `6a602de17d040836c1641df0` as the immediate endpoint rollback target, `6a60e7c06cebf50c3948c690` as the reviewed V1 Alt UI target, and `6a5e5bbef33863243b980d83` as the pre-Alt UI target. |
| Tracking / data collection / history | Applied V1 migration `20260721222627_alternative_pick_selection_state.sql` remains unchanged with RLS/browser denial and frozen-row validation/immutability. The live 2026-07-23 read returned `50` V1 rows, zero V2 rows, and zero alternative notification events; the additive V2 proof migration and column are absent. Local V2 fixtures and comparator evidence do not write production state, and no official or shared tracking/analytics table, backfill, reconstruction, deletion, or retention behavior changed. | Continue bounded V1 collection. Do not apply the V2 migration, backfill a missed checkpoint, or delete/compact evidence before the prospective fixture gate and separate Tyler approval. Version rollback must restore the exact original bundle-key state, obtain one clean V1 cycle with zero new V2 rows, and prove complete current-slate V1 coverage before restoring a V1 UI. |
