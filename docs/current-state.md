...
| 2026-07-17 decision-integrity board update | Current stage | Next decision |
| --- | --- | --- |
| Pipeline / notifications | `LIVE_READY_TO_BET_SHADOW` remains implementation-ready and default-off; `record` writes decision state plus first ready transitions to shadow evidence only. It neither creates nor suppresses notification events. | Collect five normal slates of ready-shadow overlap and false-candidate evidence before any send/suppression review. |
| UI | Same-line trust is implementation-ready: alternate-line rows remain visible as context, while automatic Log Bet prefill requires the model K line. | Verify same-line defaulting and alternate-line context on the next normal slate. |
