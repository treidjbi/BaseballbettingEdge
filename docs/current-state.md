...
| Tracking / data collection / history | `docs/research/market-tracker-map.md`, `2026-08-18-bounded-retention-audit.md`, `2026-08-25-active-provider-compaction-finalizer.md`, `docs/research/2026-08-21-boltodds-retention-closure-preflight.md`, `2026-08-20-retention-compaction-and-historical-extras-design.md`, `2026-08-20-retention-compaction-and-historical-extras.md`, compact outcome outputs, live-market audits | Canonical research and market/live/provider trackers remain active, and BoltOdds stays retired. The bounded Netlify history path is live. August 20 remains repaired at `11`, `7-4`, `+2.317154u`. On August 24, approved job `job-da68v42jnfac73a6si6g` restored all nine missing August 19 lock-backed rows: the canonical slate is now `18`, `8-10`, `-3.752883u`, all nine recovered rows remain `data_complete=0`, and bounded Supabase/Netlify reads agree. The frozen Alt V2 record from the first healthy July 24 slate is `38-46`, `-16.751729u` on `84` graded rows, plus one void and zero missing. The August 25 SELECT-only retention audit completed all `360/360` April 28-July 26 provider/date partitions. Retired BoltOdds is exact, but stale 20,000-row compaction prefixes leave `105` active-provider partitions requiring `34,861` aggregate upserts: PropLine `63/15,387` and TheRundown `42/19,474`. Main commit `643c8c10` is live on `bbe-live-layer`; the `19:20Z` due cycle naturally hit the new fail-closed 20,000-row ceiling and still finished successfully at `19:21:26Z`. Supabase compact state remained `876` rows / `20,000` represented snapshots with latest update `18:41:16Z`, proving no partial publication. That compaction-only exception still collapsed the combined market-state result and caused Alt V2 to skip as `prerequisite_failed`. Local branch `codex/compaction-alt-failure-isolation` now contains only the compact sub-result, keeps the warning explicit, and lets legacy/V2 Alt proceed while preserving fail-closed top-level/current/official/lock/timing/ready-to-bet prerequisites; it is not merged or deployed. The live compactor ceiling remains unchanged, and active-provider repair execution remains blocked. A narrow linked proof under query contract `611dd4eb...` classifies all `24` PropLine May 17 extras as exact May 16 carryover with zero unpreserved groups. The raw PropLine webhook inbox contains `440,717` rows/about `435.6 MiB`; it has no FK/view/trigger dependency, but active writes and `10` recent unprocessed rows were present. A completed physical backup at `2026-08-25T05:39:27.316Z` is newer than the repairs; PITR remains disabled. | Review the bounded Alt-isolation branch before any separate deployment decision. Design complete active-provider compaction as a low-frequency finalizer rather than enlarging the frequent live read. Require a new exact repair design and approval before any upsert. Webhook deletion remains `NO-GO` until the inbox is quiescent despite the newer backup. Raw retention deletion, vacuum/reclamation, receiver changes, and notification changes remain separate closed gates. |
