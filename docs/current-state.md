...
| Lane | Confirmed evidence | Next decision / blocker |
| --- | --- | --- |
| Pipeline / infrastructure | The September 2 scheduled Render grading run failed and published nothing. GitHub repair run `33650131043` then proved unsafe because it read stale repository history, selected May 30, and failed before push or publication. The authenticated hydrated Render service `bbe-pipeline-grading` was therefore triggered once and finished successfully at `2026-09-02T16:24:13Z`. Supabase publication run `manual-render-grading-2026-09-01-20260902T162358Z` completed with five artifacts: September 1 dated slate, index, params, performance, and picks history. Netlify `get-artifact` returned the same fresh hashes and `2026-09-02T16:23:58.854002Z` publication time. Preview, full, lock, live-layer, and current-slate artifacts remained healthy. | Do not retry GitHub grading or patch only its staging list. A usable GitHub grading fallback needs a separate artifact-hydration design and proof. The separate September 2 post-grading research run failed on an HTTP 502 while reading a remote dated artifact; no research rerun was inferred from the grading-only approval. Rerun that read-only service only with separate approval, or verify the next natural cycle. |
| Model | September 1 canonical locked outcomes are repaired: all `14/14` locked rows graded `6-8` for `-3.665684u`; the other nine history rows were unlocked and correctly remain outside performance scoring. Performance and params were regenerated at `2026-09-02T16:23:57Z` with `2,352` closed picks and a `1,887`-row complete-data calibration sample. Grading-dependent hosted research remains stale because its earlier scheduled run failed before the Gate C builder. | Hold every model/change gate until the separate research refresh succeeds. Treat the repaired official performance/history as current, but do not infer a Gate C, Alt V2, CLV, market-anchor, bet-selection, threshold, or staking decision from it. |
