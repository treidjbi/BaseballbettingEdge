...
3. **PropLine shadow/fallback evidence**: PropLine polling can support fallback,
   ...
   delivery evidence, but webhook-derived movement remains shadow-only and
   must not drive production odds, picks, notifications, or provider promotion
...
| PropLine webhook processor noisy or ambiguous | Signed webhook rows lack sportsbook key or create duplicate/low-value movement facts | Webhook evidence pollutes shadow reads or future alert logic | `propline_webhook_deliveries`, `line_movement_events`, dedupe keys, metadata.bookmaker_key_missing | Keep webhook processing shadow-only; compare against polling/BoltOdds; disable `LIVE_PROCESS_PROPLINE_WEBHOOKS` if noisy | Do not promote webhook rows to notifications or provider source without book-level proof |
