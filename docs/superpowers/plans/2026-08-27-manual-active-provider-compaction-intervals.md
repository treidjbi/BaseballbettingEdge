...
- After Tyler separately approved the exact `2026-06-14` compact-only tranche,
  the execution wrote `172` PropLine and `266` TheRundown compact rows (`438`
...
- A fresh post-write preview returned zero missing, mismatched, unexpected, or
  remaining upsert rows. PropLine is exact at `194` rebuilt/existing rows and
  TheRundown is exact at `336` rebuilt/existing rows for the target date.
- After Tyler separately approved the exact `2026-06-15` compact-only tranche,
  the execution wrote `112` PropLine and `314` TheRundown compact rows (`426`
  total). Both providers reported `execution_status=confirmed` and
  `post_write_exact=true`.
- A fresh post-write preview returned zero missing, mismatched, unexpected, or
  remaining upsert rows. PropLine is exact at `124` rebuilt/existing rows and
  TheRundown is exact at `337` rebuilt/existing rows for the target date.
- After Tyler separately approved the exact `2026-06-16` compact-only tranche,
  the execution wrote `192` PropLine and `500` TheRundown compact rows (`692`
  total). Both providers reported `execution_status=confirmed` and
  `post_write_exact=true`.
- A fresh post-write preview returned zero missing, mismatched, unexpected, or
...
- After Tyler separately approved the exact `2026-06-18` compact-only tranche,
  the execution wrote `44` PropLine and `224` TheRundown compact rows (`268`
  total). Both providers reported `execution_status=confirmed` and
  `post_write_exact=true`.
- A fresh post-write preview returned zero missing, mismatched, unexpected, or
  remaining upsert rows. PropLine is exact at `112` rebuilt/existing rows and
  TheRundown is exact at `288` rebuilt/existing rows for the target date.
- After Tyler separately approved the exact `2026-06-19` compact-only tranche,
  the execution wrote `180` PropLine and `450` TheRundown compact rows (`630`
  total). Both providers reported `execution_status=confirmed` and
  `post_write_exact=true`.
- A fresh post-write preview returned zero missing, mismatched, unexpected, or
  remaining upsert rows. PropLine is exact at `190` rebuilt/existing rows and
  TheRundown is exact at `477` rebuilt/existing rows for the target date.
- After Tyler separately approved the exact `2026-06-20` compact-only tranche,
  the execution wrote `188` PropLine and `534` TheRundown compact rows (`722`
  total). Both providers reported `execution_status=confirmed` and
  `post_write_exact=true`.
- A fresh post-write preview returned zero missing, mismatched, unexpected, or
  remaining upsert rows. PropLine is exact at `204` rebuilt/existing rows and
  TheRundown is exact at `643` rebuilt/existing rows for the target date.
- After Tyler separately approved the exact `2026-06-21` compact-only tranche,
  the execution wrote `159` PropLine and `397` TheRundown compact rows (`556`
  total). Both providers reported `execution_status=confirmed` and
  `post_write_exact=true`.
- A fresh post-write preview returned zero missing, mismatched, unexpected, or
  remaining upsert rows. PropLine is exact at `202` rebuilt/existing rows and
  TheRundown is exact at `464` rebuilt/existing rows for the target date.
- The next read-only checkpoint for `2026-06-22` passed with no evidence
  blockers and no write attempt. It identified `163` PropLine and `447`
  TheRundown compact rows to repair (`610` total). That exact tranche remains
