-- SELECT-only preview for a possible repair of the 17 stale TheRundown
-- 2026-07-15 compact groups exposed by the v2-005 deletion gate.
-- This query performs no writes and returns aggregate evidence only.

with observed_mismatches as (
  select
    ms.id,
    ms.observed_at,
    mpr.slate_date,
...
select
  ...
  count(*) filter (
    where existing_id is null
       or old_player_name is distinct from player_name
       or old_first_seen_at is distinct from first_seen_at
       or old_last_seen_at is distinct from last_seen_at
       or old_first_odds is distinct from first_odds
       or old_last_odds is distinct from last_odds
       or old_min_odds is distinct from min_odds
       or old_max_odds is distinct from max_odds
