-- Independent SELECT-only confirmation for approved tranche-v2-004.
with targets(provider, slate_date, expected_rows, expected_groups) as (
  values
    ('therundown', date '2026-07-19', 17014::bigint, 569::bigint),
    ('propline', date '2026-07-18', 41334::bigint, 610::bigint),
    ('therundown', date '2026-07-18', 24282::bigint, 686::bigint),
    ('propline', date '2026-07-17', 55232::bigint, 644::bigint),
    ('therundown', date '2026-07-17', 24064::bigint, 551::bigint)
), raw_state as (
  ...
  group by targets.provider, targets.slate_date, targets.expected_rows, targets.expected_groups
), compact_state as (
  ...
  select raw_state.*, compact_state.compact_group_count, compact_state.represented_snapshot_rows,
...
select jsonb_build_object(
  'scope_id', 'prepared_tranche_v2_004_postcheck_v1',
...
) as prepared_tranche_postcheck from checks;
